# BlockState
호출된 곳에 위치한 블록의 `BlockState` 값을 추출하는 루트테이블을 제공합니다. 바닐라 루트테이블을 변조하지 않으며, 오직 이진 탐색으로만 올바른 블록을 얻어냅니다.<br>
이진 탐색은 루트테이블의 `type:alternatives`에서 `children`에 엔트리 두개를 넣고, 그중 한쪽에 조건문을 달아줌으로써 if&else문을 흉내내어 구현했습니다. 이런 방식은 if만으로 모든것을 처리해야 하는 기존 함수트리보다 2배이상 빠르다는 장점이 있습니다.<br>
<br><br>
